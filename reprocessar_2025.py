"""
Script para processar 2025 diretamente da planilha:
1. Apaga todos os honorários de 2025
2. Processa planilha criando honorários e clientes conforme necessário
3. Se tem valor na coluna = PAGO, senão = PENDENTE com valor do primeiro mês encontrado
"""
import openpyxl
import os
import sys
...
def processar_2025():
    print("\n" + "="*60)
    print("   PROCESSAMENTO DIRETO DA PLANILHA 2025")
    print("="*60)
    
    conn = get_connection()
    cursor = conn.cursor()
    
    # 1. Apagar todos os honorários de 2025
    print("\n🗑️  Apagando todos os honorários de 2025...")
    cursor.execute("DELETE FROM honorarios WHERE ano = 2025")
    ...
    print(f"  ✅ {len(clientes_db)} clientes no banco")
    
    # 3. Carregar planilha
    print("\n📂 Carregando planilha 2025.xlsx...")
    wb = openpyxl.load_workbook('2025.xlsx', data_only=True)
    ws = wb['2025']
    print(f"  ✅ Planilha carregada ({ws.max_row} linhas)")
    
    # 4. PRIMEIRO: encontrar todas as linhas com código numérico
    print("\n🔍 Buscando linhas com código numérico...")
    linhas_clientes = []
    ...
    for linha in linhas_clientes:
        ...
        # Encontrar valor padrão do cliente (primeiro valor encontrado)
        valor_padrao = None
